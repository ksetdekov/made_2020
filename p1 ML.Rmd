...
```
```{r preprocessing}
pp <- preProcess(training[,-c(1, 6, 8, 18, 20, 22)], 
                     method="pca", pcaComp=20)
pp
transformed <- predict(pp, newdata = training[,-c(1, 6, 8, 18, 20, 22)])
head(transformed)
...
### compare them
```{r dependson=c(-1)}
transform_testing  <- predict(pp, newdata = testing[,-c(1, 6, 8, 18, 20, 22)])
...
```{r}
trans_test  <- predict(pp, newdata = test[,-c(1, 6, 8, 18, 20, 22)])
